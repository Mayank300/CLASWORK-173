...
AFRAME.registerComponent("markerhandler", {
  ...
  handleRatings: async function (dish) {
    var tNum;
    tableNumber <= 9 ? (tNum = `T0${tableNumber}`) : (tNum = `T${tableNumber}`);
    var orderSum = await this.getOrderSummary(tNum);
    var currentOrders = Object.keys(orderSum.current_orders);
    if (currentOrders.length > 0 && currentOrders === dish.id) {
      ...
      var ratButton = document.getElementById("#save-rating-button");
      ...
      swal({
        icon: "warning",
        title: "Opps!!",
        text: "Unable to find Dish in your order list!",
        timer: 2500,
        buttons: false,
      });
    }
